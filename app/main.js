...
var App = function () {
  ...
  _createClass(App, [{
    ...
    value: function run_adventure() {
      ...
      if (this.pointer.state === App.INPUT_ACTIVE) {
        ...
        if (dist >= App.INPUT_DISTANCE_SENSITIVITY * this.sizeRatioY) {
          ...
          if (dist >= App.INPUT_DISTANCE_SENSITIVITY * this.sizeRatioY * 2) {
            this.pointer.start.x = this.pointer.now.x - Math.cos(angle) * App.INPUT_DISTANCE_SENSITIVITY * this.sizeRatioY * 2;
            this.pointer.start.y = this.pointer.now.y - Math.sin(angle) * App.INPUT_DISTANCE_SENSITIVITY * this.sizeRatioY * 2;
          }
        }
      } else {
        //Key input
      ...
      for (var i = 0, actor; actor = this.actors[i]; i++) {
        if (actor == this.player) {
          ...
          //----------------
          var tileX = Math.floor(actor.x / App.TILE_SIZE);
          var tileY = Math.floor((actor.y - this.tileYOffset) / App.TILE_SIZE);
          ...
          if (this.tiles[tileY][tileX] === App.TILE_TYPE_AIR) {
            if (actor.speed.y < 0) {
              //It's hard to climb if it ain't a vine!
              actor.speed.y /= 2;
            }
      ...
      for (var y = 0; y < this.tiles.length; y++) {
        for (var x = 0; x < this.tiles[y].length; x++) {
          var tileSprite = this.tiles[y][x] === App.TILE_TYPE_VINE ? App.SPRITE_DATA.VINE_TILE : App.SPRITE_DATA.AIR_TILE;
          this.context.drawImage(this.sprites.img, tileSprite.srcX, tileSprite.srcY, tileSprite.width, tileSprite.height, Math.floor(x * App.TILE_SIZE) + tileSprite.offsetX, Math.floor(y * App.TILE_SIZE + this.tileYOffset) + tileSprite.offsetY, tileSprite.width, tileSprite.height);
        }
      }
      ...
      for (var i = this.actors.length - 1, actor; actor = this.actors[i]; i--) {
        ...
        //Draw sprite
        this.context.drawImage(this.sprites.img, actorSprite.srcX, actorSprite.srcY, actorSprite.width, actorSprite.height, Math.round(actor.x) + actorSprite.offsetX, Math.round(actor.y) + actorSprite.offsetY, actorSprite.width, actorSprite.height);
      }
      ...
      if (this.pointer.state === App.INPUT_ACTIVE) {
        this.context.fillStyle = App.COLOUR_SHADOW;
        this.context.beginPath();
        this.context.arc(this.pointer.start.x, this.pointer.start.y, App.INPUT_DISTANCE_SENSITIVITY * this.sizeRatioY, 0, 2 * Math.PI);
        this.context.fill();
        this.context.closePath();
      }
    ...
    value: function addTileRow() {
      ...
      //--------------------------------
      for (var i = 2; i < difficultyLevel; i++) {
        if (Utility.randomInt(1, 6) === 1) {
          var newActor = new Actor(Actor.TYPE_SPIKES, Utility.randomInt(App.TILE_SIZE, this.width - App.TILE_SIZE), -App.TILE_SIZE / 2);
          newActor.speed.y = App.TILE_SCROLL_SPEED + App.GRAVITY * Utility.randomInt(1, 3) * 0.5;
          this.actors.push(newActor);
        }
      }
      //--------------------------------
